...
#' ##
#' crop_racmoM_to_shelf <- function(racmoData, extent,
#'                                  rectangularExtent = FALSE, ...) {
#'   #' Crop monthly RACMO data to an ice shelf
#'   #'
#'   #' @description This function is a simple wrapper around
#'   #'   `racmoR::crop_racmo()` and `racmoR::read_racmoM_data()` to spatially crop
#'   #'   monthly RACMO SpatRasters.
...
#'
#'   # Code -----------------------------------------------------------------------
#'   croppedRacmo <- crop_racmoM(racmoData, extent, rectangularExtent,
#'                              returnOnly = "shelves", ...)
#'
#'   return(croppedRacmo)
#' }
#'
#' ##
#' crop_racmoD_to_shelf <- function(racmoData, extent,
#'                                  rectangularExtent = FALSE, ...) {
#'   #' Crop daily RACMO data to an ice shelf
#'   #'
#'   #' @description This function is a simple wrapper around
#'   #'   `racmoR::crop_racmo()` and `racmoR::read_racmoM_data()` to spatially crop
#'   #'   daily RACMO SpatRasters.
...
#'
#'   # Code -----------------------------------------------------------------------
#'   croppedRacmo <- crop_racmoD(racmoData, extent, rectangularExtent,
#'                              returnOnly = "shelves", ...)
#'
#'   return(croppedRacmo)
#' }
...
#'
#' #' ##
#' #' crop_racmoM <- function(racmoData, extent, ...) {
#' #'   #' Crop monthly RACMO data to a spatial extent
#' #'   #'
#' #'   #' @description This function is a simple wrapper around `terra::crop()`,
#' #'   #'   `racmoR::read_racmoM_data()` and `racmoR::get_extent()` to spatially
#' #'   #'   reduce a RACMO SpatRaster.
#' #'   #'
#' #'   #' @param racmoData SpatRaster: The RACMO data to crop. Can be either a
#' #'   #'   variable name, in which case raw monthly RACMO data is read in; or an
#' #'   #'   existing SpatRaster of monthly RACMO data.
#' #'   #' @param extent The area to crop the RACMO data to; fed directly into
#' #'   #'   `racmoR::get_extent()` so see there for details.
#' #'   #' @param ... Any other arguments that can be fed into `get_extent()`. Be
#' #'   #'   aware that setting the rectangularExtent argument as "FALSE" makes no
#' #'   #'   difference, and a rectangularExtent will always be used for cropping.
#' #'   #'
#' #'   #' @export
#' #'
#' #'   # Code -----------------------------------------------------------------------
...
#' #'
#' #' ##
#' #' crop_racmo <- function(racmoData, extent, ...) {
#' #'   #' Crop RACMO data to a spatial extent
#' #'   #'
#' #'   #' @description This function is a simple wrapper around `terra::crop()` and
#' #'   #'   `racmoR::get_extent()` to spatially reduce a RACMO SpatRaster.
#' #'   #'
#' #'   #' @param racmoData SpatRaster: The RACMO data to crop. It must be an
#' #'   #'   existing SpatRaster; if not, use `crop_racmoD()` or `crop_racmoM()`.
#' #'   #' @param extent The area to crop the RACMO data to; fed directly into
#' #'   #'   `racmoR::get_extent()` so see there for details.
#' #'   #' @param ... Any other arguments that can be fed into `get_extent()`. Be
#' #'   #'   aware that setting the rectangularExtent argument as "FALSE" makes no
