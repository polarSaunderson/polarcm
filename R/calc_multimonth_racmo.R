calc_multiMonth_racmo <- function(racmoData,
                                  months, annual,
                                  FUN, ...) {
  #' Calculate RACMO values over a multiple month period each year summer
  #'
  ...
  #'   JJA precipitation. The date of each returned layer is the median value of
  #'   the constituent layers that it was computed from. Only layers with the
  ...
  #'   median value of the constituent layers that it was computed from. Only
  ...
  for (ii in periods) {
    ...
    if (length(unique(units)) == 1) {    # Units must match to combine layers
      ...
      terra::time(iiFun) <- median(terra::time(iiData))
...
calc_annual_racmo <- function(racmoData,
                              ...
                              FUN,
                              ...) {
  #' Calculate RACMO values over a multiple month period each year
  #'
  #' @description Often, it is necessary to look at RACMO values across multiple
  ...
  #'   date of each returned layer is the median value of the constituent layers
  #'   that it was computed from. Only layers with the same units are processed
  #'   together, and only years when all requested months are available are
  #'   returned.
  #'
...
calc_austral_racmo <- function(racmoData,
                               ...
                               australSplit = 3,
                               ...) {
  #' Calculate RACMO values over a multiple month period each austral year
  #'
  #' @description Often, it is necessary to look at RACMO values across multiple
  #'   months, but across years. For example, what is the total DJF
  #'   precipitation each austral summer? Or what is the average wind speed in
  #'   DJF? This function takes the input RACMO data, and creates new
  #'   SpatRasters based on the requested 'months' and the 'FUN' (function)
  #'   arguments.
  #'
  #'   The following example further illustrates this function. Take a monthly
  #'   RACMO dataset of precipitation, including all months each year from
  #'   1979--2018 (i.e. 12 months * 40 years = 480 layers). Set 'months' as
  #'   c(12, 1, 2) (i.e. DJF), and FUN as "sum". The returned SpatRaster will
  #'   contain 40 layers, with each one being that year's total DJF
  #'   precipitation. The date of each returned layer is the median value of the
  #'   constituent layers that it was computed from. Only layers with the same
  #'   units are processed together, and only austral years when all requested
  #'   months are available are returned.
  #'
  #' @inheritParams calc_multiMonth_racmo
  #'
...
# The two functions below are the first approach; they are kept for posterity.

calc_multiMonth_data_each_year <- function(racmoData,
                                        months,
                                        FUN, ...) {
  #' Calculate RACMO values over a multiple month period each year
  #'
  ...
  #' @param racmoData SpatRaster: The RACMO data to use with 'FUN'. It must be
  #'   an existing SpatRaster.
  #' @param months vector: Which month/s to include each year? Input can be the
  #'   month number (e.g. 12) or the month name, either in full ("December",
  #'   "december") or abbreviated ("Dec", "dec"). Multiple months can be input
  #'   at once (e.g. c(12, 1, 2)), but do not try to mix strings and numbers in
  #'   the vector.
  #' @param FUN Which function should be applied? Examples include "mean", "sd",
  ...
  #' @param ... Any arguments that should be passed to the 'FUN' function.
  #'
  #' @seealso calc_multiMonth_data_each_summer
  ...
  for (ii in years) {
    ...
    if (length(unique(units)) == 1) {    # Units must match to combine layers
      ...
      names(iiFun) <- gsub("height=0",
                           ...
                           names(iiData)[[1]])
      # Store outside the loop
      racmoCubud[[which(years == ii)]] <- iiFun
    }
  }
...
calc_multiMonth_data_each_summer <- function(racmoData,
                                         months, FUN,
                                         australSplit = 3,
                                         ...) {
  #' Calculate RACMO values over a multiple month period each austral year
  ...
  #' @param ... Any arguments that should be passed to the 'FUN' function.
  #'
  #' @seealso calc_multiMonth_data_each_year
  ...
  # Basic data
  racmoData  <- subset_racmoM_by_month(racmoData, months,
                                       removeIncomplete = australSplit)
  racmoDates <- terrapin::get_terra_dates(racmoData)
  summers      <- unique(racmoDates$summer)

  # Apply FUN each year
  for (ii in summers) {
    iiData <- subset_racmoM_by_summer(racmoData, ii,
    ...
    if (length(unique(units)) == 1) {    # Units must match to combine layers
      ...
      names(iiFun) <- gsub("height=0",
                           ...
                           names(iiData)[[1]])
      # Store outside the loop
      racmoCubud[[which(summers == ii)]] <- iiFun
    }
  }
